...
class WeatherProvider extends ChangeNotifier{
  ...
  void _getCurrentData() async{
    final uri = Uri.parse('https://api.openweathermap.org/data/2.5/weather?lat=$latitude&lon=$longitude&$unit=matric&appid=b567eba6c40f017dff2045dd2f1914cb');
    try{
  ...
  _getForecastData() async {
    final uri = Uri.parse('https://api.openweathermap.org/data/2.5/forecast?lat=$latitude&lon=$longitude&$unit=matric&appid=b567eba6c40f017dff2045dd2f1914cb');
    try{
      final response = await get(uri);
      final map = jsonDecode(response.body);
      if(response.statusCode == 200){
